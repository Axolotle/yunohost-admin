...
type FileReturnType<T extends FileModelValue> = T extends {
  ...
  : T extends {
    ...
    : null
type FormValueReturnType<T extends PossibleFormValues> = T extends boolean
  ? 0 | 1
...
>(
  ...
  }) as FormatFormDataReturnType<R>
}

type FormatFormDataReturnType<R> = Promise<
  | Partial<{
      [k in keyof R as R[k] extends undefined | null ? never : k]: R[k]
    }>
  | { [k in keyof R as R[k] extends undefined | null ? never : k]: R[k] }
